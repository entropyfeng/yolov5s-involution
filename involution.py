...
class Involution2dNew(nn.Module):
    def __init__(self,
                 channels,
                 kernel_size,
                 stride):
        super(Involution2dNew, self).__init__()
        self.kernel_size = kernel_size
        self.stride = stride
        ...
        self.group_channels = 16
        self.groups = self.channels // self.group_channels
        self.conv1=nn.Sequential(nn.Conv2d(in_channels=channels,out_channels=channels//reduction_ratio),nn.BatchNorm2d(),nn.ReLU())
...
class Involution2d(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # [1,256,16,16]->[1,512,16,16]
        after_init=self.initial_mapping(input)
        # [1,512,16,16]->[1,4608,64]
        input_unfolded = self.unfold(after_init)
        # [1,4608,64]->[1,16,32,9,8,8]
        input_unfolded = input_unfolded.view(batch_size, self.groups, self.out_channels // self.groups,
                                             self.kernel_size[0] * self.kernel_size[1],
                                             out_height, out_width)

        #-----------------------------------------------------------
        #[1,256,16,16]->[1,128,8,8]
        after_sigma_mapping=self.sigma_mapping(self.reduce_mapping(self.o_mapping(input)))
        # Generate kernel
        #[1,128,8,8]->[1,144,8,8]
        kernel = self.span_mapping(self.sigma_mapping(self.reduce_mapping(self.o_mapping(input))))
        # [1, 1, 3, 9, 512, 512]->[1,1,1,9,512,512]
        ...
        #[1,1,1,9,512,512]*[1,1,3,9,512,512]=[1,1,3,9,512,512]
        temp=kernel*input_unfolded
        #[1,1,3,9,512,512]->[1,1,3,512,512]
        output = (kernel * input_unfolded).sum(dim=3)
        # Reshape output
        # [1,1,3,512,512]->[1,3,512,512]
        output = output.view(batch_size, -1, output.shape[-2], output.shape[-1])
        n_,c_,h_,w_=output.shape
        output.permute(0,1)
        myxxx=[output[x] for x in range(c_)]
        myxxx=torch.cat(myxxx,0)
        myxxx.permute(0,1)
        return myxxx
