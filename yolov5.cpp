...
struct Profiler : public IProfiler {
    ...
    // store each layers forward time into vector
    virtual void reportLayerTime(const char *layerName, float ms) PLUGIN_NOEXCEPT {
        //std::cout<<"call"<<std::string (layerName)<<" -> "<<ms<<std::endl;
...
ICudaEngine *
build_involution_A(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt, float &gd,
                   float &gw, std::string &wts_name) {
    const auto explicitBatch = 1U << static_cast<uint32_t>(NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);
    INetworkDefinition *network = builder->createNetworkV2(explicitBatch);
    // the default batch size is 1
    // Create input tensor of shape {batch size,3, INPUT_H, INPUT_W} with name INPUT_BLOB_NAME
    ...
    auto inconv10 = inConvNew(network, weightMap, *bottleneck_csp9->getOutput(0), get_width(512, gw), 7, 1, 1,
                              "model.10");
    ...
    auto inconv14 = inConvNew(network, weightMap, *bottleneck_csp13->getOutput(0), get_width(256, gw), 7, 1, 1,
                              "model.14");
    ...
    auto inconv18 = inConvNew(network, weightMap, *bottleneck_csp17->getOutput(0), get_width(256, gw), 7, 2, 1,
                              "model.18");
    ...
    auto inconv21 = inConvNew(network, weightMap, *bottleneck_csp20->getOutput(0), get_width(512, gw), 7, 2, 1,
                              "model.21");
...
ICudaEngine *
build_involution_B(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt, float &gd,
                   float &gw, std::string &wts_name) {
    const auto explicitBatch = 1U << static_cast<uint32_t>(NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);
    INetworkDefinition *network = builder->createNetworkV2(explicitBatch);
    // the default batch size is 1
    ...
    /* ------ yolov5 backbone------ */
    auto focus0 = focusWithBs(network, weightMap, *data, 3, get_width(64, gw), 3, "model.0");
    auto inconv1 = inConvNew(network, weightMap, *focus0->getOutput(0), get_width(128, gw), 7, 2, 1, "model.1");
    auto bottleneck_CSP2 = C3(network, weightMap, *inconv1->getOutput(0), get_width(128, gw), get_width(128, gw),
                              get_depth(3, gd), true, 1, 0.5, "model.2");
    auto inconv3 = inConvNew(network, weightMap, *bottleneck_CSP2->getOutput(0), get_width(256, gw), 7, 2, 1,
                             "model.3");
    auto bottleneck_csp4 = C3(network, weightMap, *inconv3->getOutput(0), get_width(256, gw), get_width(256, gw),
                              get_depth(9, gd), true, 1, 0.5, "model.4");
    auto inconv5 = inConvNew(network, weightMap, *bottleneck_csp4->getOutput(0), get_width(512, gw), 7, 2, 1,
                             "model.5");
    auto bottleneck_csp6 = C3(network, weightMap, *inconv5->getOutput(0), get_width(512, gw), get_width(512, gw),
                              get_depth(9, gd), true, 1, 0.5, "model.6");
    auto inconv7 = inConvNew(network, weightMap, *bottleneck_csp6->getOutput(0), get_width(1024, gw), 7, 2, 1,
                             "model.7");
    auto spp8 = SPP(network, weightMap, *inconv7->getOutput(0), get_width(1024, gw), get_width(1024, gw), 5, 9, 13,
                    "model.8");
...
ICudaEngine *
build_involution_C(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt, float &gd,
                   float &gw, std::string &wts_name) {
    const auto explicitBatch = 1U << static_cast<uint32_t>(NetworkDefinitionCreationFlag::kEXPLICIT_BATCH);
    INetworkDefinition *network = builder->createNetworkV2(explicitBatch);
    // the default batch size is 1
...
build_engine_p6(unsigned int maxBatchSize, IBuilder *builder, IBuilderConfig *config, DataType dt, float &gd, float &gw,
    ...
    auto c3_4 = C3(network, weightMap, *conv3->getOutput(0), get_width(256, gw), get_width(256, gw), get_depth(9, gd),
                   true, 1, 0.5, "model.4");
    ...
    auto spp10 = SPP(network, weightMap, *conv9->getOutput(0), get_width(1024, gw), get_width(1024, gw), 3, 5, 7,
                     "model.10");
    auto c3_11 = C3(network, weightMap, *spp10->getOutput(0), get_width(1024, gw), get_width(1024, gw),
                    get_depth(3, gd), false, 1, 0.5, "model.11");
...
void APIToModel(unsigned int maxBatchSize, IHostMemory **modelStream, bool &is_p6, float &gd, float &gw,
                std::string &wts_name, bool isInvolution , const std::string &involutionType) {
    // Create builder
    ...
    } else if (isInvolution) {
        if (involutionType == "A") {
            engine = build_involution_A(maxBatchSize, builder, config, DataType::kFLOAT, gd, gw, wts_name);
        } else if (involutionType == "B") {
            engine = build_involution_B(maxBatchSize, builder, config, DataType::kFLOAT, gd, gw, wts_name);
        } else if (involutionType == "C") {
            engine = engine = build_involution_C(maxBatchSize, builder, config, DataType::kFLOAT, gd, gw, wts_name);
        } else {
            std::cerr << "unsupport involution type" << std::endl;
        }
...
void doInference(IExecutionContext &context, cudaStream_t &stream, void **buffers, float *input, float *output) {
    CUDA_CHECK(cudaMemcpyAsync(buffers[0], input, BATCH_SIZE * 3 * INPUT_H * INPUT_W * sizeof(float),
                               cudaMemcpyHostToDevice, stream));
...
bool parse_args(int argc, char **argv, std::string &wts, std::string &engine, bool &is_p6, float &gd, float &gw,
                std::string &img_dir, bool &isInvolution, std::string &involutionType, bool &isTestSpeed) {
    if (argc < 3) return false;
    if (std::string(argv[1]) == "-s" && (argc == 5 ||argc==3|| argc == 7 || argc == 6)) {
        wts = std::string(argv[2]);
        engine = std::string(argv[3]);
        auto net = std::string(argv[4]);
        if (argc == 6) {
            auto tempInvolutionType = std::string(argv[5]);
            if (tempInvolutionType == "A" || tempInvolutionType == "B" || tempInvolutionType == "C") {
                involutionType = tempInvolutionType;
                isInvolution = true;
            } else {
                return false;
            }
    ...
    } else if (std::string(argv[1]) == "-dtest" && argc == 4) {
       auto involutionFlag= std::string(argv[2]);
        if (involutionFlag=="in"){
            isInvolution= true;
            engine = std::string(argv[3]);
            isTestSpeed = true;
        }

    } else if(std::string(argv[1]) == "-dtest" && argc == 3){
        isTestSpeed= true;
        engine=std::string(argv[2]);
    } else{
        return false;
...
int testModelForwardSpeed(std::string &engine_name,bool  isInvolution) {
    CUDART_DEVICE(DEVICE);
    //buffers on cuda
    void *buffers[2];
    std::ifstream file(engine_name, std::ios::binary);
    if (!file.good()) {
        std::cerr << "read target engine" << engine_name << " error!" << std::endl;
        return -1;
    }
    ...
    auto *data = new float[BATCH_SIZE * 3 * INPUT_H * INPUT_W];
    for (int i = 0; i < 3 * INPUT_H * INPUT_W; i++) {
        data[i] = 1.0f + (float) i;
    }
    auto *res = new float[BATCH_SIZE * OUTPUT_SIZE];
    ...
    for (int i = 0; i < TIMING_ITERATIONS; i++) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(buffers[0], data, BATCH_SIZE * 3 * INPUT_H * INPUT_W * sizeof(float),
        ...
        CUDA_CHECK(cudaMemcpyAsync(res, buffers[1], BATCH_SIZE * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost));
    }
...
int main(int argc, char **argv) {
    ...
    bool is_p6 = false;
    bool isTestSpeed = false;
    bool isInvolution = false;
    float gd = 0.0f, gw = 0.0f;
    std::string img_dir;
    std::string involutionType;

    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir, isInvolution, involutionType,
                    isTestSpeed)) {
        std::cerr << "arguments not right!" << std::endl;
        std::cerr << "./yolov5 -s [.wts] [.engine] [s/m/l/x/s6/m6/l6/x6 or c/c6 gd gw]  // serialize model to plan file"
                  << std::endl;
        std::cerr
                << "./yolov5 -s [.wts] [.engine] [s/m/l/x/s6/m6/l6/x6 or c/c6 gd gw] [A/B/C] // serialize model to plan file"
                << std::endl;
        std::cerr << "./yolov5 -d [.engine] ../samples  // deserialize plan file and run inference" << std::endl;
        std::cerr << "./yolov5 -dtest [.engine]  // use mock data test forward time" << std::endl;
        return -1;
    }
    ...
    if (!wts_name.empty()) {
        ...
        APIToModel(BATCH_SIZE, &modelStream, is_p6, gd, gw, wts_name, isInvolution,involutionType);
    ...
    if (isTestSpeed && !engine_name.empty()) {

        testModelForwardSpeed(engine_name, isInvolution);
        return 0;
    }
...
void testSingleImg() {
    ...
    CUDART_DEVICE(DEVICE);
    const int inputSize = BATCH_SIZE * 3 * INPUT_H * INPUT_W;
    const int outputSize = BATCH_SIZE * OUTPUT_SIZE;
    auto *prob = new float[outputSize];
    assert(prob);
    // prepare input data ---------------------------
    auto *data = new float[inputSize];
    assert(data);
    void *buffers[2];
    // Create GPU buffers on device
    CUDA_CHECK(cudaMalloc(&buffers[0], inputSize * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&buffers[1], outputSize * sizeof(float)));
    cudaStream_t stream;
    CUDA_CHECK(cudaStreamCreate(&stream));
    std::string engine_name = "/opt/myproject/yolov5-RT/yolov5s-inconv-A-mask.engine";
    // deserialize the .engine and run inference
    std::ifstream file(engine_name, std::ios::binary);
    if (!file.good()) {
        std::cerr << "read " << engine_name << " error!" << std::endl;
    ...
    std::string imgName = "yourImageName";
    std::string outImgName = "yourOutputImage";
    cv::Mat img = cv::imread(imgName);
    if (img.empty()) {
        std::cout << "the img " << imgName << "is empty" << std::endl;
    }
    ...
    for (int i = 0; i < TIMING_ITERATIONS; i++) {
        ...
        CUDA_CHECK(cudaMemcpyAsync(buffers[0], data, inputSize * sizeof(float), cudaMemcpyHostToDevice, stream));

        context->executeV2(buffers);
        CUDA_CHECK(cudaMemcpyAsync(prob, buffers[1], BATCH_SIZE * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost,
                                   stream));
    }
    auto end = std::chrono::system_clock::now();
    std::cout << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
